...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // 3.2 บันทึกประวัติ Transaction ทุกครั้งที่เพิ่มสิ่งของใหม่ (ไม่ว่าจะเริ่มต้นด้วยจำนวน 0 หรือไม่ก็ตาม)
        $logRemark = ($initial_stock > 0) ? 'รับยอดยกมาเริ่มต้น' : 'ลงทะเบียนเพิ่มสิ่งของใหม่ในระบบ';
        $logStmt = $pdo->prepare("INSERT INTO inventory_transactions (item_id, transaction_type, quantity, remark, created_by) VALUES (?, 'CREATE', ?, ?, ?)");
        $logStmt->execute([$item_id, $initial_stock, $logRemark, $created_by]);
